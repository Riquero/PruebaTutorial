...
public class Reina extends Pieza {
	...
	public Reina(final int posicionPieza, final Bando bandoDeLaPieza) {
		super(posicionPieza, bandoDeLaPieza, TipoDePieza.REINA);
	}
	...
	private static boolean exclusionDePrimeraColumna(final int posicionActual, final int posicionCandidata) {
		return UtilidadesTablero.PRIMERA_COLUMNA[posicionActual]
				&& (posicionCandidata == -9 || posicionCandidata == 9 || posicionActual == 7);
	}
	...
	private static boolean exclusionDeOctavaColumna(final int posicionActual, final int posicionCandidata) {
		return UtilidadesTablero.OCTAVA_COLUMNA[posicionActual]
				&& (posicionCandidata == -7 || posicionCandidata == 1 || posicionActual == 9);
	}

	@Override
	public String toString() {
		return tipoDePieza.REINA.toString();
	}

	@Override
	public Reina moverPieza(Movimiento movimiento) {		
		return new Reina(movimiento.getCoordenadaDeDestino(), movimiento.getPiezaMovida().getBando());
	}
}
